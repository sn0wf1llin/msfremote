...
ENV					RUBY_VERSION=${ruby_version} MSF_DB_USER=${msf_db_user} MSF_DB_PASSWORD=${msf_db_password}
...
RUN 	echo '#!/bin/bash' > /usr/local/bin/init.sh && \
			printf "\nsource /usr/local/rvm/scripts/rvm" >> /usr/local/bin/init.sh && \
			printf "\nservice postgresql start && echo \"PostgreSQL started [OK]\" || echo \"PostgreSQL started [FAIL]\"" >> /usr/local/bin/init.sh && \
			printf "\necho \"Got <\$TEST_RESOURCE_FILE_LINK> ... \"" >> /usr/local/bin/init.sh && \
			printf "\nrm -f /tmp/tests/*" >> /usr/local/bin/init.sh && \
			printf "\ncurl -sSL \$TEST_RESOURCE_FILE_LINK --output /tmp/tests/test.rc" >> /usr/local/bin/init.sh && \
